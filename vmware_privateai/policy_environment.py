...
from vmware_privateai.config import CONFIG_ENV_VAR, CONFIG_FILE, load_config
...
#
# The cache stats the variable's path to decide staleness, but which file is
# *read* is decided by load_config -> resolve_config_path, the same call the
# CLI and the connection make, so the environment is judged from the file the
# write connects with.
_cached_config = mtime_cached_loader(CONFIG_ENV_VAR, CONFIG_FILE, load_config)
