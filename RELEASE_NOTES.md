## v1.2.2 — stopping the MCP server logs out its vCenter session
...
audit row under that tool's name (15 commands, `@audited`). Commands that reach nothing remote say so, with a reason (4, `@cli_local`).
