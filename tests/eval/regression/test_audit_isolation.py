...
``vmware_privateai`` is imported: ``HOME`` and ``OPS_HOME`` both point into a
...
database; it does *not* cover ``vmware_privateai.notify.audit.AuditLogger``, whose
default path is the ``~``-relative string ``~/.vmware-privateai/audit.log`` and
...
def test_the_skill_json_lines_log_is_inside_the_sandbox() -> None:
    ...
    assert _under(AuditLogger()._log_file, SANDBOX_HOME)

    # This skill's CLI is a package, so the module-scope logger lives in _common.
    from vmware_privateai.cli._common import _audit as cli_audit

    assert _under(cli_audit._log_file, SANDBOX_HOME), (
