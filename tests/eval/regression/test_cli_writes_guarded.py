...
the same enforcement @vmware_tool gives the MCP surface — so ``vmware-privateai
gpu vgpu-assign`` run through Bash is authorized and audited to
~/.vmware/audit.db exactly like the ``vgpu_assign`` MCP tool.
...
the Python function name (``vgpu_assign_cmd``), so a rule written against
``vgpu_assign`` refused the agent and let the same reconfigure through the CLI,
...
# cannot satisfy the floor. vgpu_assign_cmd is this skill's only write.
_MUST_DERIVE: tuple[str, ...] = ("vgpu_assign_cmd",)
_WRITE_FLOOR = 1
...
def _ops_refs(tree: ast.AST) -> tuple[dict[str, str], set[str]]:
    ...
    aliased call (the tools alias ``gpu_utilization as _util``) resolves to the
...
def test_one_deny_rule_naming_the_mcp_tool_stops_the_cli_command(tmp_path, monkeypatch):
    """The point of the naming rule, end to end: deny ``vgpu_assign`` and the CLI is refused too.
    ...
    ``vgpu_assign_cmd`` and this rule matched nothing on that surface.
    """
    import vmware_policy.policy as pm
    from typer.testing import CliRunner
    from vmware_policy import PolicyDenied

    from vmware_privateai.cli import app
    from vmware_privateai.cli import gpu as cli_gpu

    rules = tmp_path / "rules.yaml"
    rules.write_text(
        'deny:\n  - name: no-vgpu-assign\n    operations: ["vgpu_assign"]\n    reason: frozen\n',
        encoding="utf-8",
    )
    ...
    monkeypatch.setattr(cli_gpu, "_get_connection", _must_not_connect)
    result = CliRunner().invoke(app, ["gpu", "vgpu-assign", "vm-1", "grid_a100-4c", "--dry-run"])
    ...
    assert "Denied by policy" in result.output and "no-vgpu-assign" in result.output, result.output
